...
const MovieCard: React.FC<MovieCardProps> = ({ data }) => {
  ...
  return (
    <div className='group bg-dark-netfuix col-span relative h-[109px] md:h-[131px] lg:h-[130px] xl:h-[121px] 2xl:h-[165px] rounded-md mt-8'>
      <Image
      ...
      />
      <div className='opacity-0 absolute top-0 transition duration-300 z-10 invisible sm:visible delay-200 w-full scale-0 md:group-hover:scale-100 lg:group-hover:scale-110 xl:group-hover:scale-125 2xl:group-hover:scale-[1.45] group-hover:-translate-y-[5vw] group-hover:opacity-100'>
        <div
          onClick={() => router.push(`/watch/${data?.id}`)}
          className='relative cursor-pointer object-cover transition duration shadow-md rounded-t-md w-full h-[79px] md:h-[101px] lg:h-[100px] xl:h-[91px] 2xl:h-[135px] overflow-hidden'
        >
          <Image
            src={data?.thumbnailUrl}
            alt='Movie'
        ...
        <div className='z-10 bg-[#181818] p-2 lg:py-3 lg:px-4 absolute w-full transition shadow-md rounded-b-md'>
          <div className='flex flex-row items-center gap-3'>
            ...
            <div
              ...
              className='cursor-pointer group/item w-6 h-6 lg:w-7 lg:h-7 bg-[rgba(42,42,42,.6)] border-[hsla(0,0%,100%,.5)] border-[0.12rem] rounded-full flex justify-center items-center transition hover:border-white ml-auto'
            >
              <BsChevronDown size={16} className='text-white' />
            </div>
          </div>
